...
def load_table_config(column_mapping_yaml_path, table_name):
    """Load table config (DDL and columns) from YAML."""
    with open(column_mapping_yaml_path) as f:
        mappings = yaml.safe_load(f)
    table_cfg = mappings["tables"].get(table_name)
    if not table_cfg:
        raise ValueError(f"Table config for '{table_name}' is missing in '{column_mapping_yaml_path}'.")
    ...
    columns = {k.lower(): {"csv_col": v["csv_col"].lower(), "df_dtype": v["df_dtype"]} for k, v in columns.items()}
    return columns


def render_ddl(column_mapping_yaml_path, table_name, schema_name):
    with open(column_mapping_yaml_path) as f:
        mappings = yaml.safe_load(f)
    table_cfg = mappings["tables"].get(table_name)
